  ...
  $.ajax({
    ...
    success: function(recs){
      if(recs=="Sorry! The movie you requested is not in our database. Please check the spelling or try with some other movies"){
        // Movie not in similarity database - still show movie details without recommendations
        $('.fail').css('display','none');
...
function show_details(movie_details,arr,movie_title,my_api_key,movie_id){
  ...
  movie_videos = get_movie_videos(movie_id,my_api_key);
  ...
  details = {
    'title':movie_title,
      ...
      'trailer_key':movie_videos.trailer_key,
      'teaser_key':movie_videos.teaser_key,
      'movie_id':movie_id,
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      $('#autoComplete').val('');
      // Hide trending section when showing movie details
...
function get_movie_posters(arr,my_api_key){
  var arr_poster_list = []
  if (arr.length === 0) return arr_poster_list;
  ...
  for(var m in arr) {
    $.ajax({
      ...
      error: function(){
        console.log("Could not fetch poster for: " + arr[m]);
      },
    })
  }
...
// getting trailer and teaser videos for the movie
function get_movie_videos(movie_id,my_api_key){
  var trailer_key = '';
  var teaser_key = '';
    ...
    success: function(video_data){
      if(video_data.results.length > 0){
        for(var i=0; i<video_data.results.length; i++){
          if(video_data.results[i].type === 'Trailer' && video_data.results[i].site === 'YouTube' && trailer_key === ''){
            trailer_key = video_data.results[i].key;
          }
          if(video_data.results[i].type === 'Teaser' && video_data.results[i].site === 'YouTube' && teaser_key === ''){
            teaser_key = video_data.results[i].key;
          }
        }
      }
    },
  ...
  return {trailer_key:trailer_key, teaser_key:teaser_key};
}
